...
static bool insideTriangle(float x, float y, const Vector3f *_v) {
    ...
    Vector3f aq(x - _v[0].x(), y - _v[0].y(), 0);
    Vector3f bq(x - _v[1].x(), y - _v[1].y(), 0);
    Vector3f cq(x - _v[2].x(), y - _v[2].y(), 0);

    float res = ab.cross(aq)[2];
    if (res * bc.cross(bq)[2] >= 0 && res * ca.cross(cq)[2] >= 0) {
        return true;
    }
