...
void rst::rasterizer::rasterize_triangle(const Triangle &t, const std::array<Eigen::Vector3f, 3> &view_pos) {
    ...
    float min_x = FLT_MAX, min_y = FLT_MAX, max_x = -1, max_y = -1;
    for (auto &point: v) {
    ...
    // iterate all pixels in triangle, update depth with z-buffer then set color
    for (int i = (int) min_x; i <= (int) max_x; i++) {
        for (int j = (int) min_y; j <= (int) max_y; j++) {
            // ignore points outside this triangle
            if (!insideTriangle(i + 0.5, j + 0.5, t.v)) {
                continue;
            }

            // get interpolated depth value of this point
            auto[alpha, beta, gamma] = computeBarycentric2D(i + 0.5, j + 0.5, t.v);
            float Z = 1.0 / (alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w());
            float zp = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
            zp *= Z;

            // z-buffering, update depth buffer
            int index = get_index(i, j);
            if (zp <= depth_buf[index]) {
                // interpolate color, normal and texture coords
                Vector3f interpolated_color =
                        alpha * t.color[0] / v[0].w() + beta * t.color[1] / v[1].w() + gamma * t.color[2] / v[2].w();
                interpolated_color *= Z;

                Vector3f interpolated_normal =
                        alpha * t.normal[0] / v[0].w() + beta * t.normal[1] / v[1].w() + gamma * t.normal[2] / v[2].w();
                interpolated_normal *= Z;

                Vector2f interpolated_texcoords =
                        alpha * t.tex_coords[0] / v[0].w() + beta * t.tex_coords[1] / v[1].w() + gamma * t.tex_coords[2] / v[2].w();
                interpolated_texcoords *= Z;

                Vector3f interpolated_shadingcoords =
                        alpha * view_pos[0] / v[0].w() + beta * view_pos[1] / v[1].w() + gamma * view_pos[2] / v[2].w();
